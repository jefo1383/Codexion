...
/*   Updated: 2026/09/15 16:18:11 by jfoeller         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	init_dongles(t_sim *sim)
{
	...
	i = 0;
	sim->dongles = ft_calloc(sizeof(t_dongle), sim->config.nb_coders);
	if (!sim->dongles)
		return (false);
...
bool	init_coders(t_sim *sim)
{
	...
	i = 0;
	sim->coders = ft_calloc(sizeof(t_coder), sim->config.nb_coders);
	if (!sim->coders)
		return (false);
	while (i < sim->config.nb_coders)
	{
...
{
	heap->requests = ft_calloc(sizeof(t_request), max_capacity);
	if (!heap->requests)
		return (false);
	heap->nb_requests = 0;
